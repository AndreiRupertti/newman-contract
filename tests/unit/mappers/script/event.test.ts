...
import exec from "@mappers/script/exec";
import {
...
describe("Mapper to build Event", () => {
  beforeAll(() => {
    mockExec.mockReturnValue(validPostmanDummyTestExec);
  ...
  it("should build postman test script correctly", () => {
    ...
    expect(buildExec).toBeCalledWith({
      type: EventTypes.TEST,
      exec: dummyTestExec
    });
  ...
  it("should build postman test script correctly", () => {
    (buildExec as jest.Mock).mockReturnValueOnce(
      validPostmanDummyPreRequestExec
    );
    const { script } = buildEvent({
      type: EventTypes.PRE_REQUEST,
      exec: dummyPreRequestExec
    });
